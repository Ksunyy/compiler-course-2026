...
void replaceWithLeftShift(llvm::BinaryOperator *oldInst, llvm::Value *valToShift, 
                          llvm::ConstantInt *powerOf2Const, llvm::IRBuilder<> &builder) {
    uint64_t shiftAmt = powerOf2Const->getValue().logBase2();
    llvm::Value *shiftConst = builder.getIntN(powerOf2Const->getBitWidth(), shiftAmt);
    
    llvm::Value *newShift = builder.CreateShl(valToShift, shiftConst, "leftShift");
...
void replaceWithRightShift(llvm::BinaryOperator *oldInst, llvm::Value *valToShift, 
                           llvm::ConstantInt *powerOf2Const, llvm::IRBuilder<> &builder, bool isArithmetic) {
    uint64_t shiftAmt = powerOf2Const->getValue().logBase2();
    llvm::Value *shiftConst = builder.getIntN(powerOf2Const->getBitWidth(), shiftAmt);
...
struct replaceWithShiftPass : llvm::PassInfoMixin<replaceWithShiftPass> {
    llvm::PreservedAnalyses run(llvm::Function &func, llvm::FunctionAnalysisManager &) {
        ...
        for (auto &baseBlock : func) {
            for (auto instruction = baseBlock.begin(), end = baseBlock.end(); instruction != end; ) {
